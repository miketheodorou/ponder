...
apps/mobile/
 ...
 │ ├── CatalogueSheet.tsx # Bottom sheet shell — Reanimated slide + drag-to-dismiss handle + internal nav stack
 ...
 │ ├── QuoteDetail.tsx # Read-only detail view: quote, metadata rows, tags, notes, journal entries section
 │ └── index.ts
 ├── data/
...
- **Internal nav stack** lives inside `CatalogueSheet`, not as a router stack. Two animated panes (list and detail) sit absolutely-positioned inside the sheet body; a single `stackDepth` shared value (0 → root, 1 → pushed) drives both panes' `translateX` and the underlying pane's opacity. We re-implemented the iOS-style push (list slides 30% left + dims to 0.4, detail slides in from the right) instead of nesting a real `Stack` inside the sheet — which fights expo-router's modal/back-button behavior and adds a frame of jank. Why a hand-roll: only one push depth, one screen pair, and we need the sheet's drag-to-dismiss to still close the *whole* sheet regardless of nav depth (a nested router would intercept the gesture).
- **Pop-transition tracking.** When the user pops back to the list, we keep the most recent detail entry rendered via a `lastDetail` state until the pane finishes animating off-screen — otherwise the pane unmounts the moment `top` goes null and the user sees an empty rectangle slide right. The `lastDetail` is harmless to leave stale; it's overwritten the next time a detail is pushed.
...
- **QuoteDetail** is a read-only view of one quote — back-button NavHeader (inline, since only one screen uses it today; extract to `components/NavHeader.tsx` when JournalEntry lands), quote in serif `serif3xl`, metadata rows (Author/Book/Page/Saved), tag chips, notes (italic serif when present, italic sans-faint placeholder when absent), and a journal section that lists linked entries with a `+ New entry` outline button at the bottom. The "+ tag" affordance and journal interactions (open entry, new entry) are wired as TODOs — the journal-entry editor is the next pass.
